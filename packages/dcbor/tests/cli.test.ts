...
describe('CLI Tests - Map Command (vs dcbor CLI)', () => {
  ...
  test('text_keys', () => {
    const cliOutput = runDcbor(['map', '--out', 'diag', '"key1"', '"value1"', '"key2"', '"value2"']);
...
describe('CLI Tests - Complex Structures (vs dcbor CLI)', () => {
  test('deeply_nested_array', () => {
    const cliOutput = runDcborWithInput(['--in', 'diag', '--out', 'hex'], '[[[1, 2], [3, 4]], [[5, 6], [7, 8]]]');
    const tsOutput = toHex([[[1, 2], [3, 4]], [[5, 6], [7, 8]]]);
    expect(tsOutput).toBe(cliOutput);
  });

  test('complex_map', () => {
    const cliOutput = runDcborWithInput(['--in', 'diag', '--out', 'hex'], '{"name": "Alice", "age": 30, "active": true}');
    const map = CborMap.new();
    map.set('name', 'Alice');
    map.set('age', 30);
    map.set('active', true);
  ...
  test('mixed_nested_structures', () => {
    const cliOutput = runDcborWithInput(['--in', 'diag', '--out', 'hex'], '{1: [1, 2, 3], 2: {"nested": true}}');
